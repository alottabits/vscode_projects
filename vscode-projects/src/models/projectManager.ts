...
import { DataRecord, DataFrame } from './dataframe';
...
export class ProjectManager {
  ...
  public async archiveProject(id: string): Promise<void> {
    ...
    if (project) {
      this.settings.projects = this.settings.projects.filter(p => p.id !== id);
      this.settings.archives.push(project);
      await this.saveSettings();
    }
  }
  ...
  public async queryProject(project: ProjectDefinition): Promise<DataFrame> {
    ...
    try {
      if (project.dataSource.kind === 'folder') {
        ...
        for (const file of files) {
          try {
            ...
            errors.push(new Error(`Error processing ${file}: ${err}`));
          }
        }
      ...
      errors.push(new Error(`Error querying project: ${err}`));
    }
